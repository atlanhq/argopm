 ...
 */
const installGlobal = function (
    packageName,
    registry,
    namespace,
    cluster,
    options,
    argoWorkflowControllerConfigMapName,
    argoNamespace
) {
...
const install = function (
    ...
    return npmInstall(dirPath, packageName, registry, npmSaveParam)
        ...
        .then((dirs) => {
            ...
            return Promise.each(dirs, async function (dir) {
                if (dir.split("/").slice(-1)[0].startsWith("@")) {
                    return listDirs(dir).then((innerDirs) => {
                        return Promise.each(innerDirs, async function (innerDir) {
                            const s3Uploader = new S3(
                                argoWorkflowControllerConfigMapName,
                                argoNamespace,
                                packageNameFromPath(innerDir)
                            );
                            await s3Uploader.initialize();
                            await s3Uploader.uploadStaticFiles(innerDir);
                ...
                const s3Uploader = new S3(argoWorkflowControllerConfigMapName, argoNamespace, packageNameFromPath(dir));
                await s3Uploader.initialize();
                await s3Uploader.uploadStaticFiles(dir);
        ...
        .then(async (_) => {
            ...
            const s3Uploader = new S3(argoWorkflowControllerConfigMapName, argoNamespace, parentPackageName);
            await s3Uploader.initialize();
            await s3Uploader.uploadStaticFiles(dirPath);

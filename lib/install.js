 ...
 */
const installGlobal = function (packageName, registry, namespace, cluster) {
    const dirPath = `/tmp/argopm/${packageName}`
    let mainPackageName = packageName;
    return install(packageName, registry, namespace, false, cluster, dirPath).then(name => {
 ...
 */
const install = function (packageName, registry, namespace, save, cluster, dirPath = process.cwd()) {
            ...
            if (dir.split("/").slice(-1)[0].startsWith("@")) {
                return listDirs(dir).then(innerDirs => {
                    return Promise.each(innerDirs, function(innerDir) {
                        const k8sInstaller = new K8sInstaller(innerDir, namespace, parentPackageName, registry);

...
class Package {
    ...
    packageInfo(namespace) {
        ...
        return this.dataflowPipelines(namespace)
            .then((pipelines) => {
                ...
                info += pipelinesInfo;
                return this.numaflowPipelines(namespace);
            })
            .then((pipelines) => {
                let pipelinesInfo = blue(bright("\nNumaflow Pipelines: \n"));
                pipelines.forEach((pipeline) => {
    ...
    delete(cluster, namespace) {
        ...
        return this.dependencies(cluster)
            ...
            .then((_) => {
                console.log(`Deleting dataflow pipelines for package ${this.metadata.name}`);
                return capturedThis.deleteDataflowPipelines(namespace);
            })
            .then((_) => {
                console.log(`Deleting numaflow pipelines for package ${this.metadata.name}`);
                return capturedThis.deleteNumaflowPipelines(namespace);
            })
            .then((_) => {
            ...
            .then((_) => {
                ...
                if (cluster) {
                    ...
                    return customK8sApi.deleteClusterCustomObject({
                        group: constants.ARGO_K8S_API_GROUP,
                        version: constants.ARGO_K8S_API_VERSION,
                        plural,
                        name: capturedThis.metadata.name,
                    });
                }
                return customK8sApi.deleteNamespacedCustomObject({
                    group: constants.ARGO_K8S_API_GROUP,
                    version: constants.ARGO_K8S_API_VERSION,
                    namespace: capturedThis.metadata.namespace,
                    plural,
                    name: capturedThis.metadata.name,
                });
            });
        ...
        if (cluster) {
            ...
            return customK8sApi
                .listClusterCustomObject({
                    ...
                    version: constants.ARGO_K8S_API_VERSION,
                    plural,
                    labelSelector: this.info.getDependencyLabel(),
                })
                .then((response) => {
    ...
    configMaps(namespace) {
        return coreK8sApi
            .listNamespacedConfigMap({ namespace, labelSelector: this.info.getPackageLabel() })
            .then((response) => {
    ...
    deleteConfigMaps(namespace) {
        return this.configMaps(namespace).then((configMaps) => {
            return Promise.each(configMaps, function (configMap) {
                ...
                return coreK8sApi.deleteNamespacedConfigMap({ name: metadata.name, namespace: metadata.namespace });
            });
        });
    }
    ...
    secrets(namespace) {
        return coreK8sApi
            .listNamespacedSecret({ namespace, labelSelector: this.info.getPackageLabel() })
            .then((response) => {
                return response.items;
            });
    }
    ...
    deleteSecrets(namespace) {
        return this.secrets(namespace).then((secrets) => {
            return Promise.each(secrets, function (secret) {
                ...
                return coreK8sApi.deleteNamespacedSecret({ name: metadata.name, namespace: metadata.namespace });
            });
        });
    }

    /**
     * Returns all dataflow pipleines associated with the package
     * @returns {Promise<Array<Object>>}
     */
    dataflowPipelines(namespace) {
        let plural = constants.ARGO_PIPELINES_PLURAL;
        return customK8sApi
            .listNamespacedCustomObject({
                group: constants.ARGO_DATAFLOW_K8S_API_GROUP,
                version: constants.ARGO_K8S_API_VERSION,
                namespace,
                plural,
                labelSelector: this.info.getPackageLabel(),
            })
            .then((response) => {
                return response.items;
            });
    ...
    /**
     * Deletes all dataflow pipelines associated with the package
     * @returns {Promise<Any>}
     */
    deleteDataflowPipelines(namespace) {
        let plural = constants.ARGO_PIPELINES_PLURAL;
        return this.dataflowPipelines(namespace).then((pipelines) => {
    ...
    /**
     * Returns all numaflow pipelines associated with the package
     * @returns {Promise<Array<Object>>}
     */
    numaflowPipelines(namespace) {
        let plural = constants.ARGO_PIPELINES_PLURAL;
        return customK8sApi
            .listNamespacedCustomObject({
                group: constants.ARGO_NUMAFLOW_K8S_API_GROUP,
                version: constants.ARGO_K8S_API_VERSION,
                namespace,
                plural,
                labelSelector: this.info.getPackageLabel(),
    ...
    /**
     * Deletes all numaflow pipelines associated with the package
     * @returns {Promise<Any>}
     */
    deleteNumaflowPipelines(namespace) {
        let plural = constants.ARGO_PIPELINES_PLURAL;
        return this.numaflowPipelines(namespace).then((pipelines) => {
            return Promise.each(pipelines, function (pipeline) {
                const metadata = pipeline.metadata;
                return customK8sApi.deleteNamespacedCustomObject({
                    group: constants.ARGO_NUMAFLOW_K8S_API_GROUP,
        ...
        return this.arguments
            ...
            .then((workflow) => {
                return customK8sApi.createNamespacedCustomObject({
                    ...
                    version: constants.ARGO_K8S_API_VERSION,
                    namespace,
                    plural: constants.ARGO_WORKFLOWS_PLURAL,
                    body: workflow,
                });
            });
    }
    ...
    if (cluster) {
        ...
        return customK8sApi
            .listClusterCustomObject({
                ...
                version: constants.ARGO_K8S_API_VERSION,
                plural,
                labelSelector: Package.getInstallerLabel(),
            })
            .then((response) => {

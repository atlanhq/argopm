...
class Package {
    ...
    delete(cluster) {
        ...
        return this.dependencies(cluster).then(dependencyPackages => {
            ...
            return capturedThis.cronWorkflows(cluster);
        }).then(cronWorkflows => {
            return Promise.each(cronWorkflows, function (cronWorkflow) {
                const metadata = cronWorkflow.metadata;
                customK8sApi.deleteNamespacedCustomObject(constants.ARGO_K8S_API_GROUP,
                    constants.ARGO_K8S_API_VERSION, metadata.namespace,
                    constants.ARGO_CRON_WORKFLOW_PLURAL, metadata.name
                );
            });
     ...
     */
     cronWorkflows(cluster) {
        if (cluster) {
            return customK8sApi.listClusterCustomObject(constants.ARGO_K8S_API_GROUP, constants.ARGO_K8S_API_VERSION,
                constants.ARGO_CRON_WORKFLOW_PLURAL, null, null, null, this.info.getPackageLabel()).then(response => {
                return response.body.items;
            })
        }
        return customK8sApi.listNamespacedCustomObject(constants.ARGO_K8S_API_GROUP, constants.ARGO_K8S_API_VERSION, 
            this.metadata.namespace, constants.ARGO_CRON_WORKFLOW_PLURAL, null, null, null, this.info.getPackageLabel()).then(response => {
            return response.body.items;
        })
    }
...
Package.info = function (namespace, packageName, cluster, isPipeline) {
    ...
    if (cluster) {
        return customK8sApi.listClusterCustomObject(constants.ARGO_K8S_API_GROUP, constants.ARGO_K8S_API_VERSION, 
            ARGO_TEMPLATE_PLURAL, null, null, null, PackageInfo.getPackageLabel(packageName)).then(response => {
...
Package.list = function (namespace, cluster, isPipeline) {
    ...
    if (cluster) {
        return customK8sApi.listClusterCustomObject(constants.ARGO_K8S_API_GROUP, constants.ARGO_K8S_API_VERSION, 
            ARGO_TEMPLATE_PLURAL, null, null, null, Package.getInstallerLabel()).then(response => {

...
class Package {
    ...
    packageInfo() {
        ...
        return this.pipelines().then(pipelines => {
            ...
            cronWorkflows.forEach(cronWorkflow => {
                ...
                cronWorkflowInfo += `- Name: ${yellow(cronWorkflow.metadata.name)}, Schedule: ${lightCyan(cronString)}, Timezone: ${lightCyan(cronTimezone)}\n`
            });
        ...
        return this.dependencies(cluster).then(dependencyPackages => {
            ...
            if (cluster) {
                ...
                return customK8sApi.deleteClusterCustomObject(constants.ARGO_K8S_API_GROUP, constants.ARGO_K8S_API_VERSION, 
                    plural, capturedThis.metadata.name
                );
            }
            return customK8sApi.deleteNamespacedCustomObject(constants.ARGO_K8S_API_GROUP,
                constants.ARGO_K8S_API_VERSION, capturedThis.metadata.namespace, plural, capturedThis.metadata.name
            )
        })
    }
    ...
    dependencies(cluster) {
        ...
        if (cluster) {
            ...
            return customK8sApi.listClusterCustomObject(constants.ARGO_K8S_API_GROUP, constants.ARGO_K8S_API_VERSION,
                plural, null, null, null, this.info.getDependencyLabel()).then(response => {
        ...
        return customK8sApi.listNamespacedCustomObject(constants.ARGO_K8S_API_GROUP, constants.ARGO_K8S_API_VERSION,
            this.metadata.namespace, plural, null, null, null, this.info.getDependencyLabel()).then(response => {
            return this.getDependentPackagesFromListResponse(response)
        })
    }
     ...
     */
    deleteConfigMaps() {
        return this.configMaps().then(configMaps => {
     ...
     */
     secrets() {
        return coreK8sApi.listNamespacedSecret(this.metadata.namespace, null, null, null, null,
            this.info.getPackageLabel()).then(response => {
            return response.body.items;
        })
    }
     ...
     */
     deleteSecrets() {
        return this.secrets().then(secrets => {
    ...
    pipelines() {
        ...
        return customK8sApi.listNamespacedCustomObject(constants.ARGO_DATAFLOW_K8S_API_GROUP, constants.ARGO_K8S_API_VERSION, 
            this.metadata.namespace, plural, null, null, null, this.info.getPackageLabel()).then(response => {
            return response.body.items;
        })
    }
     ...
     deletePipelines() {
        ...
        return this.pipelines().then(pipelines => {
            return Promise.each(pipelines, function (pipeline) {
                ...
                return customK8sApi.deleteNamespacedCustomObject(constants.ARGO_DATAFLOW_K8S_API_GROUP, constants.ARGO_K8S_API_VERSION, 
                    metadata.namespace, plural, metadata.name)
            })
        })
    }
     ...
     */
     deleteCronWorkflows() {
        let plural = `${constants.ARGO_CRON_WORKFLOW_KIND.toLowerCase()}s`
        return this.cronWorkflows().then(cronWorkflows => {
            return Promise.each(cronWorkflows, function (cronWorkflow) {
                const metadata = cronWorkflow.metadata;
                return customK8sApi.deleteNamespacedCustomObject(constants.ARGO_K8S_API_GROUP, constants.ARGO_K8S_API_VERSION, 
                    metadata.namespace, plural, metadata.name)
            })
        })
    }
...
Package.info = function (namespace, packageName, cluster) {
    ...
    if (cluster) {
        ...
        return customK8sApi.listClusterCustomObject(constants.ARGO_K8S_API_GROUP, constants.ARGO_K8S_API_VERSION, 
            plural, null, null, null, PackageInfo.getPackageLabel(packageName)).then(response => {
    ...
    return customK8sApi.listNamespacedCustomObject(constants.ARGO_K8S_API_GROUP, constants.ARGO_K8S_API_VERSION, 
        namespace, plural, null, null, null, PackageInfo.getPackageLabel(packageName)).then(response => {
            const items = response.body.items;
            if (items.length !== 1) {
...
Package.list = function (namespace, cluster) {
    ...
    if (cluster) {
        plural = `${constants.ARGO_CLUSTER_WORKFLOW_TEMPLATES_KIND.toLowerCase()}s`
        return customK8sApi.listClusterCustomObject(constants.ARGO_K8S_API_GROUP, constants.ARGO_K8S_API_VERSION, 
            plural, null, null, null, Package.getInstallerLabel()).then(response => {
            return handleListResponse(response);
        })
    }
    return customK8sApi.listNamespacedCustomObject(constants.ARGO_K8S_API_GROUP, constants.ARGO_K8S_API_VERSION,
        namespace, plural, null, null, null, Package.getInstallerLabel()).then(response => {
            return handleListResponse(response);
    })
}

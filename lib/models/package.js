...
class Package {
    ...
    constructor(k8sYaml) {
        ...
        } else if (k8sYaml.kind == "CronWorkflow") {
            this.argoTemplatePlural = constants.ARGO_CRON_WORKFLOW_PLURAL
        }
    }
 ...
 * @returns {Promise<Package>}
 */
Package.info = function (namespace, packageName, cluster, isPipeline, isCron) {
    ...
    if (isPipeline) {
        ...
        ARGO_TEMPLATE_PLURAL = constants.ARGO_CRON_WORKFLOW_PLURAL
    } else if (cluster) {
 ...
 * @returns {Promise<[Package]>}
 */
Package.list = function (namespace, cluster, isPipeline, isCron) {
    var K8S_API_GROUP = constants.ARGO_K8S_API_GROUP, ARGO_TEMPLATE_PLURAL = constants.ARGO_WORKFLOW_TEMPLATES_PLURAL;

...
class Package {
     ...
     */
    run(args, serviceAccountName, imagePullSecrets, cluster, namespace, cronString, timeZone) {
        ...
        return this.arguments.checkRequiredArgs(runtimeArguments).then(_ => {
            ...
            if (cronString) {
                ...
                workflow.spec.schedule = cronString
                if (timeZone) workflow.spec.timezone = timeZone
            }
...
Package.info = function (namespace, packageName, cluster, isPipeline) {
    ...
    if (isPipeline) {
        K8S_API_GROUP = constants.ARGO_DATAFLOW_K8S_API_GROUPg

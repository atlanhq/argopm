 ...
 */
PackageInfo.createK8sLabelsForPackage = function (packageObject) {
    let labels = {};
    labels[constants.ARGOPM_INSTALLER_LABEL] = encode(constants.ARGOPM_INSTALLER_LABEL_VALUE);
    labels[constants.ARGOPM_LIBRARY_NAME_LABEL] = encode(packageObject.name);
 ...
 * @param {Object} packageObject
 */
PackageInfo.createK8sAnnotationsForPackage = function (packageObject, registry) {
    ...
    annotations[constants.ARGOPM_LIBRARY_AUTHOR_LABEL] = packageObject.description;
...
PackageInfo.getPackageLabel = function (packageName) {
    return `${constants.ARGOPM_LIBRARY_NAME_LABEL}=${encode(packageName)}`;
};

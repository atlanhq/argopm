...
class S3 {
    ...
    getS3ConfigFromArgo(configMapName, argoNamespace) {
        ...
        return coreK8sApi
            .readNamespacedConfigMap({ name: configMapName, namespace: argoNamespace })
            .then((argoWorkflowControllerConfigMap) => {
                const bucket = yaml.load(argoWorkflowControllerConfigMap.data.bucket);
                const region = yaml.load(argoWorkflowControllerConfigMap.data.region);

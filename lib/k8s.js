...
class K8sInstaller {
    ...
    upsertTemplate(packageName, namespace, kind, group, yamlObject, cluster) {
        ...
        if (!cluster) {
            return customK8sApi.listNamespacedCustomObject(group, ARGO_K8S_API_VERSION, namespace, plural, 
        ...
        return customK8sApi.listClusterCustomObject(group, ARGO_K8S_API_VERSION, plural, 
            null, null, null, PackageInfo.getPackageLabel(packageName)).then(response => {
                if (yamlObject['kind'] == 'CronWorkflow') {
                ...
                return K8sInstaller.handleTemplateResponse(response, namespace, plural, yamlObject, cluster, K8S_API_GROUP)
        })
    }

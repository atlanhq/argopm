...
/**
 * Creates per-request options that override the Content-Type header for patch operations.
 * In @kubernetes/client-node v1.x, patch methods auto-select json-patch+json, but we need
 * merge-patch or strategic-merge-patch depending on the resource type.
 *
 * The middleware must return Observable-like objects (with a toPromise() method) because
 * mergeConfiguration only merges options.middleware (not promiseMiddleware) for per-call options.
 * @param {"merge-patch"|"strategic-merge-patch"} patchType
 */
function patchOptions(patchType) {
    const obs = (value) => ({ toPromise: () => Promise.resolve(value) });
    return {
        middleware: [{
            pre: (context) => {
                context.setHeaderParam("Content-Type", `application/${patchType}+json`);
                return obs(context);
            },
            post: (context) => obs(context),
        }],
    };
}
...
K8sInstaller.upsertConfigMap = function (packageName, namespace, kind, group, yamlObject, cluster, forceUpdate) {
    ...
    return coreK8sApi
        .listNamespacedConfigMap({ namespace, labelSelector: PackageInfo.getPackageLabel(packageName) })
        .then((response) => {
            const name = yamlObject.metadata.name;
            ...
            if (isPresent) {
                ...
                return coreK8sApi.patchNamespacedConfigMap(
                    { name, namespace, body: yamlObject },
                    patchOptions("strategic-merge-patch")
                );
            }
            console.debug(`${name} ${kind} not present in the cluster. Installing v${newVersion}`);
            return coreK8sApi.createNamespacedConfigMap({ namespace, body: yamlObject });
        });
};
...
K8sInstaller.upsertSecret = function (packageName, namespace, kind, group, yamlObject, cluster, forceUpdate) {
    return coreK8sApi
        .listNamespacedSecret({ namespace, labelSelector: PackageInfo.getPackageLabel(packageName) })
        .then((response) => {
            const name = yamlObject.metadata.name;
            const items = response.items;
            const resource = getResourceByName(items, name);
            ...
            console.debug(`${name} ${kind} not present in the cluster. Installing v${newVersion}`);
            return coreK8sApi.createNamespacedSecret({ namespace, body: yamlObject });
        });
};
    ...
    if (!cluster) {
        return customK8sApi
            .listNamespacedCustomObject({
                group,
                version: constants.ARGO_K8S_API_VERSION,
                namespace,
                plural,
                labelSelector: PackageInfo.getPackageLabel(packageName),
            })
            .then((response) => {
...
K8sInstaller.createCustomResource = function (namespace, plural, yamlObject, cluster, apiGroup) {
    if (cluster) {
        return customK8sApi.createClusterCustomObject({
            group: apiGroup, version: constants.ARGO_K8S_API_VERSION, plural, body: yamlObject,
        });
    }
    return customK8sApi.createNamespacedCustomObject({
        group: apiGroup, version: constants.ARGO_K8S_API_VERSION, namespace, plural, body: yamlObject,
    });
};
...
K8sInstaller.patchCustomResource = function (name, namespace, plural, yamlObject, cluster, apiGroup) {
    if (cluster) {
        return customK8sApi.patchClusterCustomObject(
            { group: apiGroup, version: constants.ARGO_K8S_API_VERSION, plural, name, body: yamlObject },
            patchOptions("merge-patch")
        );
    }
    return customK8sApi.patchNamespacedCustomObject(
        { group: apiGroup, version: constants.ARGO_K8S_API_VERSION, namespace, plural, name, body: yamlObject },
        patchOptions("merge-patch")
    );
};
...
K8sInstaller.recreateCustomResource = function (name, namespace, plural, yamlObject, cluster, apiGroup) {
    if (cluster) {
        return customK8sApi
            .deleteClusterCustomObject({
                group: constants.ARGO_K8S_API_GROUP, version: constants.ARGO_K8S_API_VERSION, plural, name,
            })
            .then((_) => {
                return customK8sApi.createClusterCustomObject({
                    group: apiGroup, version: constants.ARGO_K8S_API_VERSION, plural, body: yamlObject,
    ...
    return customK8sApi
        .deleteNamespacedCustomObject({
            group: apiGroup, version: constants.ARGO_K8S_API_VERSION, namespace, plural, name,
        })
        .then((_) => {
            return customK8sApi.createNamespacedCustomObject({
                group: apiGroup, version: constants.ARGO_K8S_API_VERSION, namespace, plural, body: yamlObject,

...
class K8sInstaller {
        ...
        return fs.readdirAsync(dirPath).then(files => {
            ...
            return Promise.each(files, function (file) {
                ...
                return fs.readFileAsync(filePath, 'utf8').then(data => {
                    ...
                    const folder = dirPath.split('/').filter(function(el) { 
    ...
    addAPMLabels(yamlObject, folder, fileName) {
        ...
        const argoPMLabels = PackageInfo.createK8sLabelsForPackage(this.package)
        Object.keys(argoPMLabels).forEach(function(key) { metadata.labels[key] = argoPMLabels[key]; });

        const argoPMAnnotations = PackageInfo.createK8sAnnotationsForPackage(this.package, this.registry)
        Object.keys(argoPMAnnotations).forEach(function(key) { metadata.annotations[key] = argoPMAnnotations[key]; });
